...
export default function Home() {
  ...
  const badge = (label: string, type: 'red' | 'orange' | 'green' | 'gray') => {
    const colors = {
      ...
      orange: { bg: 'rgba(251,191,36,0.15)', color: '#fbbf24' },
  ...
  const Card = ({ icon, title, subtitle, badgeLabel, badgeType, onClick }: {
    ...
    <button onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
        <div style={{
          width: 34, height: 34, background: 'rgba(45,212,176,0.12)',
          borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 18
        }}>{icon}</div>
        {badge(badgeLabel, badgeType)}
      </div>
      <p style={{ fontSize: 15, fontWeight: 600, margin: "0 0 4px", color: "#fff" }}>{title}</p>
      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: 0 }}>{subtitle}</p>
    </button>
  )
    ...
    <div style={{ background: '#1a2332', minHeight: '100vh', padding: '1.25rem 1rem 3rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1.75rem' }}>
        <div style={{
          ...
          <span style={{ color: '#1a2332', fontSize: 15, fontWeight: 800 }}>T</span>
        </div>
        <div>
          <p style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>Estudio Tekton</p>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: 0, textTransform: 'capitalize' }}>{fecha}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', margin: '0 0 10px' }}>Área técnica</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10, marginBottom: '1.75rem' }}>
            ...
            <Card icon="✏️" title="Dibujos" subtitle={`${counts.dibujos} pendientes`}
              badgeLabel={counts.dibujos_hoy > 0 ? 'vence hoy' : 'al día'}
              badgeType={counts.dibujos_hoy > 0 ? 'orange' : 'green'}
          ...
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', margin: '0 0 10px' }}>Comercial y administración</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10, marginBottom: '1.75rem' }}>
