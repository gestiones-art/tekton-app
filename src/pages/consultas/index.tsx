...
const ESTADOS: Record<string, { label: string, color: string, bg: string }> = {
  ...
  pendiente_validacion: { label: 'Pendiente', color: '#fbbf24', bg: 'rgba(251,191,36,0.12)' },
...
export default function Consultas() {
  ...
  async function loadConsultas() {
    const { data } = await supabase
      .from('consultas')
      .select('id, numero_p, nombre, municipio, tramite, estado, created_at, motivo_cancelacion')
  ...
  const filtrados = consultas.filter(c => {
    if (filtro === 'activos') return ['pendiente', 'pendiente_validacion', 'pdte_enviar', 'enviado'].includes(c.estado)
    if (filtro === 'aceptados') return c.estado === 'aceptado'
    if (filtro === 'cerrados') return c.estado === 'rechazado' || c.estado === 'cancelado'
    return true
  })

  const pendientes = consultas.filter(c => c.estado === 'pendiente' || c.estado === 'pendiente_validacion').length
          ...
          {filtrados.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => router.push(`/consultas/${c.id}`)} style={{
                ...
                {c.motivo_cancelacion && <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', margin: '4px 0 0', fontStyle: 'italic' }}>{c.motivo_cancelacion}</p>}
              </button>
            )
          })}
        </div>
